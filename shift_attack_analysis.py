...
#                        G_add += max(((G+E+M+D)/3-(G+1)),M-(G+1)) 
#                        G += G_add
#                        D_add += (G+E+M+D)/3-(E+D+1)
#                        D += D_add
#                        M_add += -(G_add+D_add)
#                        M_add += -G_add
#                        M += M_add
    ...
    sorted_by_M_add = sorted(attack_results_2b3b, key = lambda x: attack_results_2b3b[x]['M_add']) 
    sorted_by_M_add_rel = sorted(attack_results_2b3b,
        key = lambda x: attack_results_2b3b[x]['M_add']/attack_results_2b3b[x]['M_orig'])
    min_shift = attack_results_2b3b[sorted_by_M_add[0]]
    min_shift_rel = attack_results_2b3b[sorted_by_M_add_rel[0]]
    min_M_add = attack_results_2b3b[sorted_by_M_add[0]]
    min_M_add_rel = attack_results_2b3b[sorted_by_M_add_rel[0]]    
    max_shift = attack_results_2b3b[sorted_by_M_add[len(sorted_by_M_add)-1]]
    max_shift_rel = attack_results_2b3b[sorted_by_M_add_rel[len(sorted_by_M_add_rel)-1]]    
    med_shift = attack_results_2b3b[sorted_by_M_add[(len(sorted_by_M_add)-1)//2]]
    med_shift_rel = attack_results_2b3b[sorted_by_M_add_rel[(len(sorted_by_M_add_rel)-1)//2]]
    print('\t Min. abs. shifted bandwidth (% orig. G,M,T): {:.2e} ({:.2f}%) ({:.2f}%) ({:.2f}%)'.format(min_M_add['M_add'],
        100*min_shift['M_add']/min_M_add['G_orig'], 100*min_shift['M_add']/min_M_add['M_orig'], 100*min_shift['M_add']/min_M_add['T_orig']))
#    print('\t Min. shifted bandwidth (% orig. T): {:.2e} ({:.2f}%)'.format(min_M_add['M_add'],
#        100*min_shift['M_add']/min_M_add['T_orig']))
    print('\t Med. abs. shifted bandwidth (% orig. G,M,T): {:.2e} ({:.2f}%) ({:.2f}%) ({:.2f}%)'.format(med_shift['M_add'],
        100*med_shift['M_add']/med_shift['G_orig'], 100*med_shift['M_add']/med_shift['M_orig'], 100*med_shift['M_add']/med_shift['T_orig']))
    print('\t Min. rel. shifted bandwidth (% orig. G,M,T): {:.2e} ({:.2f}%) ({:.2f}%) ({:.2f}%)'.format(min_M_add_rel['M_add'],
        100*min_shift_rel['M_add']/min_M_add_rel['G_orig'], 100*min_shift_rel['M_add']/min_M_add_rel['M_orig'], 100*min_shift_rel['M_add']/min_M_add_rel['T_orig']))
    print('\t Med. rel. shifted bandwidth (% orig. G,M,T): {:.2e} ({:.2f}%) ({:.2f}%) ({:.2f}%)'.format(med_shift_rel['M_add'],
        100*med_shift_rel['M_add']/med_shift_rel['G_orig'], 100*med_shift_rel['M_add']/med_shift_rel['M_orig'], 100*med_shift_rel['M_add']/med_shift_rel['T_orig']))

 #    print('\t Med. shifted bandwidth (% orig. M): {:.2e} ({:.2f}%)'.format(med_shift['M_add'],
#        100*med_shift['M_add']/med_shift['M_orig']))
#    print('\t Med. shifted bandwidth (% orig. T): {:.2e} ({:.2f}%)'.format(med_shift['M_add'],
#        100*med_shift['M_add']/med_shift['T_orig']))
#
#    print('\t Max. shifted bandwidth (% orig. G,M,T): {:.2e} ({:.2f}%) ({:.2f}%) ({:.2f}%)'.format(max_shift['M_add'],
#        100*max_shift['M_add']/max_shift['G_orig'], 100*max_shift['M_add']/max_shift['M_orig'], 100*max_shift['M_add']/max_shift['T_orig']))
    print('\t Max. abs. shifted bandwidth (% orig. M): {:.2e} ({:.2f}%)'.format(max_shift['M_add'],
        100*max_shift['M_add']/max_shift['M_orig']))
    print('\t Max. abs. shifted bandwidth (% orig. T): {:.2e} ({:.2f}%)'.format(max_shift['M_add'],
        100*max_shift['M_add']/max_shift['T_orig']))
    min_T_add = attack_results_2b3b[sorted_by_T_add[0]]
    print('\t Max. rel. shifted bandwidth (% orig. M): {:.2e} ({:.2f}%)'.format(max_shift_rel['M_add'],
        100*max_shift_rel['M_add']/max_shift_rel['M_orig']))
    print('\t Max. rel. shifted bandwidth (% orig. T): {:.2e} ({:.2f}%)'.format(max_shift_rel['M_add'],
        100*max_shift_rel['M_add']/max_shift_rel['T_orig']))
    min_T_add = attack_results_2b3b[sorted_by_T_add[0]]

#    max_T_add = attack_results_2b3b[sorted_by_T_add[len(sorted_by_T_add)-1]]
#    max_T_add = attack_results_2b3b[sorted_by_T_add[-1]]
...
#print(M_add, M/M_start, G_add, G/G_start)
